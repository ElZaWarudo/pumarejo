---
title: pumarejo Product Requirements
date: 2026-07-23
...
pumarejo shall let an MCP-capable agent observe and operate one local Tauri 2 application through its WebView while the developer keeps control of the operating system, mouse, keyboard, and other windows.

## Actors and stakeholders

- **Primary user:** a Tauri developer working with a coding agent.
- **Primary client:** an MCP host such as Codex that invokes pumarejo tools.
...
- One reusable npm package named `pumarejo`.
...
- FR-002. `init` shall add the embedded WebDriver dependency, register it only for debug builds, add the required capability permission, and create `.pumarejo.json`.
...
- FR-006. `remove` shall remove only changes attributable to pumarejo and shall preserve unrelated project content.
...
- FR-013. `tauri_close` shall close the WebDriver session, terminate only processes started by pumarejo, release ports, and be safe to call after partial launch failure.
...
- FR-016a. Password fields and elements marked `data-pumarejo-sensitive="true"` by the application shall never expose their current value or value-bearing text; snapshots shall return a redaction marker instead, including inside open shadow roots.
...
- NFR-001. pumarejo shall generate zero operating-system mouse and keyboard input during all supported actions.
...
- NFR-004. The MCP server and WebDriver session shall be local to the developer machine; pumarejo shall not provide a network-facing remote-control mode.
...
- BR-003. pumarejo may terminate only the application process tree it started.
...
- BR-007. The consumer project is trusted local input; pumarejo shall not execute commands received from MCP tool arguments other than the project-approved application command.
...
1. The developer runs `pumarejo init` in a Tauri 2 project.
...
1. The MCP host starts `pumarejo mcp --project .`.
2. The agent calls `tauri_launch` with `mode: visible`.
3. pumarejo returns a semantic snapshot.
...
2. pumarejo starts the application in an isolated hidden or virtual display appropriate to the certified platform.
